...
So how does this example have something with the function is lazy?
...
So let's show a recursive example when achieving StackOverflow and when our machine would crash because it can’t handle more items on the stack, so we achieve `StackOverflow.
