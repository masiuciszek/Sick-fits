...
With the recursive approach we have just asked one person and been stating in the same place in the line and get back the same result.
...
So let's show a recursive example when our machine will simply crash because it can’t handle more items on the stack, We have achieved _StackOverflow_.
...
Lets take a look on how the implementation would look like.

```ts
const insert = (head: Node, data: number) => {
...
Let's take a look at the callstack.

### Using recursion in your front end application.

We can also use recursion in our projects, and sometimes it is a much more handy, element and better tool to use then iterate over a list.
...
The true fact is that our machines using recursion under the hood, when we try to run a script that searches for folders and files on our machine.
...
We can build this with React, here is the full code example [repository](https://github.com/masiucd/Recursive-render) if you want to check out it out.

We want to stop the recursion as soon we hit our base case, if there is no more items left.
...
This tecnique was something I got inspred and learned from [Kyle Shelvin](https://kyleshevlin.com/recursive-react-components) since I had a situation at my job where I need to build something similar to this.
