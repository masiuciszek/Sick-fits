...
Lets have a look on how we work with numbers using recursion. Lets implemtn a function that takes in a number and sum upp all the values up to that number
using recursion.
...
We add the current value of `n` and add to to the recursive call where we subtract the previous value.
We will kepp adding to the call stack until we hit our bascase.
So for every function call the value of `n` will be shrinked and another stack frame will be added to call stack.
=======
...
Let’s take a look at how the implementation would look like.
